...
object OpPlaceBlock : SpellAction {
    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
        ...
        val itemUseCtx =
            env.queryForMatchingStack { it.item is BlockItem }
                ?.let {
                    UseOnContext(
                        env.world,
        ...
        override fun cast(env: CastingEnvironment) {
            ...
            if (placeeStack != null) {
                if (!IXplatAbstractions.INSTANCE.isPlacingAllowed(
                    env.world, pos, placeeStack, caster as? ServerPlayer))
                    return
                ...
                if (!placeeStack.isEmpty) {
                    ...
                    // we temporarily give the player the stack, place it using mc code, then give
                    // them the old stack back.
