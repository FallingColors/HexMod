...
public class PatternRegistryManifest {
    ...
    public static void processRegistry(@Nullable ServerLevel overworld) {
        ...
        for (var key : registry.registryKeySet()) {
            ...
            if (!HexUtils.isOfTag(registry, key, HexTags.Actions.PER_WORLD_PATTERN)) {
                ...
                if (old != null) {
                    HexAPI.LOGGER.warn(
                            "Inserted %s which has same signature as %s, overriding it."
                                    .formatted(key, old));
                }
            } else {
                perWorldActionCount++;
        ...
        HexAPI.LOGGER.info(
                ("We're on the %s! Loaded %d regular actions, %d per-world actions, and %d special"
                                + " handlers")
                        .formatted(
     ...
     */
    @Nullable
    public static Pair<SpecialHandler, ResourceKey<SpecialHandler.Factory<?>>>
    ...
    @Nullable
    public static HexPattern getCanonicalStrokesPerWorld(
