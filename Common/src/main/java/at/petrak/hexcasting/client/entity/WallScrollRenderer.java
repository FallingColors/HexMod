...
public class WallScrollRenderer extends EntityRenderer<EntityWallScroll> {
    ...
    public void render(EntityWallScroll wallScroll, float yaw, float partialTicks, PoseStack ps,
...

        {
            ...
            // Remember: CCW
            // Front face
            vertex(last, light, verts, 0, 0, dz, 0, 0, 0, 0, -1);
            vertex(last, light, verts, 0, dy, dz, 0, 1, 0, 0, -1);
            vertex(last, light, verts, dx, dy, dz, 1, 1, 0, 0, -1);
            vertex(last, light, verts, dx, 0, dz, 1, 0, 0, 0, -1);
            // Back face
            vertex(last, light, verts, 0, 0, 0, 0, 0, 0, 0, 1);
            vertex(last, light, verts, dx, 0, 0, 1, 0, 0, 0, 1);
            vertex(last, light, verts, dx, dy, 0, 1, 1, 0, 0, 1);
            vertex(last, light, verts, 0, dy, 0, 0, 1, 0, 0, 1);
            // Top face
            vertex(last, light, verts, 0, 0, 0, 0, 0, 0, -1, 0);
            vertex(last, light, verts, 0, 0, dz, 0, margin, 0, -1, 0);
            vertex(last, light, verts, dx, 0, dz, 1, margin, 0, -1, 0);
            vertex(last, light, verts, dx, 0, 0, 1, 0, 0, -1, 0);
            // Left face
            vertex(last, light, verts, 0, 0, 0, 0, 0, -1, 0, 0);
            vertex(last, light, verts, 0, dy, 0, 0, 1, -1, 0, 0);
            vertex(last, light, verts, 0, dy, dz, margin, 1, -1, 0, 0);
            vertex(last, light, verts, 0, 0, dz, margin, 0, -1, 0, 0);
            // Right face
            vertex(last, light, verts, dx, 0, dz, 1 - margin, 0, 1, 0, 0);
            vertex(last, light, verts, dx, dy, dz, 1 - margin, 1, 1, 0, 0);
            vertex(last, light, verts, dx, dy, 0, 1, 1, 1, 0, 0);
            vertex(last, light, verts, dx, 0, 0, 1, 0, 1, 0, 0);
            // Bottom face
            vertex(last, light, verts, 0, dy, dz, 0, 1 - margin, 0, 1, 0);
            vertex(last, light, verts, 0, dy, 0, 0, 1, 0, 1, 0);
            vertex(last, light, verts, dx, dy, 0, 1, 1, 0, 1, 0);
            vertex(last, light, verts, dx, dy, dz, 1, 1 - margin, 0, 1, 0);
    ...
    private static void vertex(com.mojang.blaze3d.vertex.PoseStack.Pose pose, int light, VertexConsumer verts, float x, float y,
                               float z, float u,
                               float v, float nx, float ny, float nz) {
        verts.addVertex(pose.pose(), x, y, z)
                .setColor(0xffffffff)
