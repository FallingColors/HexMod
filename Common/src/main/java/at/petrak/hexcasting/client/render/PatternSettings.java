...
public class PatternSettings {
    ...
    public PatternSettings(
            String name,
            PositionSettings posSets,
            StrokeSettings strokeSets,
            ZappySettings zapSets) {
        this.name = name;
    ...
    /**
     * Settings for positioning the pattern and defining its general size/render area. All values
     * are in 'pose units', meaning we use them directly with the pose/matrix stack given to the
     * renderer.
     *
     * <p>We do a first pass at the pattern scale using baseScale. We then make sure it's larger
     * than minWidth and minHeight. Then on each axis, if that axis is has a FIT alignment then we
     * may scale down the pattern to make sure it fits. Note that the padding is not scaled and is
     * always respected.
     */
    public record PositionSettings(
            ...
            double minHeight) {
        /**
         * Makes settings ideal for rendering in a square. This helper exists because this is the
         * most common positioning pattern.
         *
         * @param padding a value 0-0.5 for how much padding should go on each side.
         * @return a PositionSettings object in a 1x1 space with the given padding value such that
        ...
        public static PositionSettings paddedSquare(
    ...
    /**
     * Settings for stroke and dot sizings. If you want to *not* render dots or inner/outer you
     * should prefer setting alpha to 0 in PatternColors.
     */
    ...
    // we have a stroke guess getter so that we can *try* to account for the stroke size when
    // fitting the pattern.
