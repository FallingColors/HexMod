...
public class PatternTextureManager {
    ...
    public static Optional<Map<String, ResourceLocation>> getTextures(
        ...
        if (patternTexturesToAdd.containsKey(patCacheKey)) {
            ...
            for (ResourceLocation oldPatternTextureSingle : oldPatternTexture.values())
                    Minecraft.getInstance()
                            .getTextureManager()
                            .getTexture(oldPatternTextureSingle)
                            .close();
        ...
        // render a higher-resolution texture in a background thread so it eventually becomes all
        ...
        if (!inProgressPatterns.contains(patCacheKey)) {
            ...
            executor.submit(
                    () -> {
                        ...
                        // TextureManager#register doesn't look very thread-safe, so move back to
                        // the main thread after the slow part is done
                        Minecraft.getInstance()
                                .execute(
                                        () -> {
    ...
    private static Map<String, DynamicTexture> createTextures(
        ...
        HexPatternPoints staticPoints =
                HexPatternPoints.getStaticPoints(patternlike, patSets, seed);
    ...
    private static Map<String, ResourceLocation> registerTextures(
        ...
        for (Map.Entry<String, DynamicTexture> textureEntry : dynamicTextures.entrySet()) {
            ...
            ResourceLocation resourceLocation =
                    Minecraft.getInstance()
                            .getTextureManager()
                            .register(name, textureEntry.getValue());
    ...
    private static NativeImage drawLines(
        ...
        g2d.setColor(
                new Color(
                        0xFF_FFFFFF)); // set it to white so we can reuse the texture with different
        // colors
        g2d.setStroke(
                new BasicStroke(
                        unscaledLineWidth * resPerUnit,
