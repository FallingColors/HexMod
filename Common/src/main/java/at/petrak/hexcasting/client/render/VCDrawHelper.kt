...
interface VCDrawHelper {
    ...
    class Worldly(
        ...
        var lastVertMode: VertexFormat.Mode? =
            null // i guess this assumes that the vcHelper is only used once at a time? maybe
        ...
        override fun vcSetupAndSupply(vertMode: VertexFormat.Mode): VertexConsumer {
            ...
            if (provider is MultiBufferSource.BufferSource) {
                // tells it to draw whatever was here before so that we don't get depth buffer
                // weirdness
                provider.endBatch()
            }
            ...
            // Generally this would be handled by a RenderLayer, but that doesn't seem to actually
            // work here,,
            RenderSystem.setShaderTexture(0, texture)
            RenderSystem.enableDepthTest()
            RenderSystem.disableCull()
