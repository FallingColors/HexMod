...
public class ScrungledPatternsSave extends SavedData {
    ...
    public static ScrungledPatternsSave createFromScratch(long seed) {
        ...
        // this means if some hilarious funny person makes a great spell that has the same shape as
        // a normal spell
        // there might be overlap.
        // I'm going to file that under "don't do that"
        // (the number literal phial incident won't happen though because we check for special
        // handlers first now)
        for (var key : registry.registryKeySet()) {
            ...
            if (HexUtils.isOfTag(registry, key, HexTags.Actions.PER_WORLD_PATTERN)) {
                ...
                map.put(
                        scrungledPat.anglesSignature(),
                        new PerWorldEntry(key, scrungledPat.getStartDir()));
            }
        }
