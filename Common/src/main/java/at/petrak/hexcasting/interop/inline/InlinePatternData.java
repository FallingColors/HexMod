...
public class InlinePatternData implements InlineData<InlinePatternData> {
    ...
    public Style getExtraStyle() {
        ...
        HoverEvent he =
                new HoverEvent(
                        HoverEvent.Action.SHOW_ITEM, new HoverEvent.ItemStackInfo(scrollStack));
    ...
    public static Component getPatternName(HexPattern pattern) {
        try {
            PatternShapeMatch shapeMatch =
                    PatternRegistryManifest.matchPattern(pattern, null, false);
            if (shapeMatch instanceof PatternShapeMatch.Normal normMatch) {
                return HexAPI.instance().getActionI18n(normMatch.key, false);
            }
            // TODO: this doesn't actually ever hit because it errors out with server castinv env
            // stuff first :(
