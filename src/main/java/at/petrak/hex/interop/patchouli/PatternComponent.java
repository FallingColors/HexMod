...
public class PatternComponent implements ICustomComponent {
    ...
    @Override
    public void render(PoseStack poseStack, IComponentRenderContext ctx, float partialTicks, int x, int y) {
        RenderLib.drawPattern(poseStack.last().pose(), this.straightPoints, 230, 230, 230, 200);
    }

    @Override
    public void onVariablesAvailable(UnaryOperator<IVariable> lookup) {
        var dir = HexDir.valueOf(this.startdir.toUpperCase());
        this.pattern = HexPattern.FromAnglesSig(this.signature, dir);
